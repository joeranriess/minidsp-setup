###
### Why reinvent the wheel? Shamelessly pinched from Mark Kubiak at https://github.com/markubiak/python3-minidsp :).
###
...
class Board2x4HD():
    ...
    def _masterStatus(self):
        ...
        while not resp:
          # send command again
          logger.warning('response empty, retry.')
          resp = self._transport.write([0x05, 0xff, 0xda, 0x02])

        if (resp[:3] != [0x05, 0xff, 0xda]) or not (resp[4] in [0x00, 0x01]):
            logger.critical('Received unexpected response: ' + str(resp))
